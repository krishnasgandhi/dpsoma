...
if ( ! class_exists( 'WPEX_Content_Importer' ) ) {
	...
	class WPEX_Content_Importer {
		...
		public function process_xml( $demo, $import_images ) {
			...
			if ( $response === false ) {
				return new WP_Error( 'xml_import_error', esc_html__( 'Can not retrieve sample data xml file. The hosting server may be down at the momment. Please try again later. If you still have issues contact the theme developer for assistance.', 'total-theme-core' ) );
			}
