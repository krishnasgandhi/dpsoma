	...
	$output .= '<div class="' . esc_attr( $wrap_classes ) . '"' . vcex_get_unique_id( $unique_id ) . '>';
		...
		if ( ! empty( $atts[ 'heading' ] ) ) {
			...
			if ( function_exists( 'wpex_get_heading' ) ) {
				...
				$output .= '<h2 class="vcex-module-heading">' . do_shortcode( wp_kses_post( $atts[ 'heading' ] ) ) . '</h2>';
		...
		$output .= '<div class="' . esc_attr( $grid_classes ) . '"' . $grid_data . '>';
			...
			while ( $vcex_query->have_posts() ) :
				...
				$oembed_video_url = vcex_get_post_video_oembed_url( $post_id );
				$embed_url        = $oembed_video_url ? vcex_get_video_embed_url( $oembed_video_url ) : '';
											...
											$categories_output .= '<div class="' . esc_attr( $categories_classes ) . '"' . $categories_style . '>';
												...
												if ( 'true' == $show_first_category_only ) {
													...
													if ( ! vcex_validate_boolean( $latts[ 'categories_links' ] ) ) {
														...
														$categories_output .= vcex_get_first_term( $latts['post_id'], $categories_taxonomy );

													} else {

														$categories_output .= vcex_get_first_term_link( $latts['post_id'], $categories_taxonomy );

													}

												} else {

													$categories_output .= vcex_get_list_post_terms( $categories_taxonomy, vcex_validate_boolean( $latts[ 'categories_links' ] ) );
