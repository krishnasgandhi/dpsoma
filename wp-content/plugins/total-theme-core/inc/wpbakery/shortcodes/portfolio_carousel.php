	...
	class VCEX_Portfolio_Carousel_Shortcode {
		...
		public function vc_after_mapping() {
			...
			if ( is_admin() ) {
				...
				add_filter(
					'vc_autocomplete_' . $this->shortcode .'_include_categories_callback',
					'vcex_suggest_portfolio_categories'
				);
				add_filter(
					'vc_autocomplete_' . $this->shortcode .'_exclude_categories_callback',
					'vcex_suggest_portfolio_categories'
				);

				// Render autocomplete suggestions
				add_filter(
					'vc_autocomplete_' . $this->shortcode .'_include_categories_render',
					'vcex_render_portfolio_categories'
				);

				add_filter(
					'vc_autocomplete_' . $this->shortcode .'_exclude_categories_render',
					'vcex_render_portfolio_categories'
				);

				// Move content design elements into new entry CSS field
				add_filter(
					'vc_edit_form_fields_attributes_' . $this->shortcode,
