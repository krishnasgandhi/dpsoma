...
class SocialProfilesWidget extends WP_Widget {
				...
				foreach( $social_services as $key => $val ) :
					...
					if ( $link ) {
						...
						if ( function_exists( 'wpex_get_social_button_class' ) ) {
							$a_attrs['class'] .= ' ' . wpex_get_social_button_class( $style );
						}
						...
						$output .= '<li>';
							...
							$output .= '>';
								...
								if ( ! defined( 'TOTAL_THEME_ACTIVE' ) ) {
									$output .= '<span class="ttc-social-widget-label">' . esc_attr( $name ) . '</span>';
								} else {
	...
	public function scripts( $hook ) {
		...
		wp_enqueue_style(
			'wpex-social-widget',
			TTC_PLUGIN_DIR_URL . 'assets/css/social-widget.css',
			false,
			'1.0'
		);
		...
		wp_enqueue_script(
			'wpex-social-widget',
			TTC_PLUGIN_DIR_URL . 'assets/js/social-widget.js',
			array( 'jquery' ),
			'1.0',
			true
		);
