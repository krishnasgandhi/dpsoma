...
if ( ! function_exists( 'wpex_pagination' ) ) { // MUST KEEP CHECK SO USERS CAN OVERRIDE
	...
	function wpex_pagination( $query = '', $echo = true ) {
		...
		if ( $total > 1 ) {
			...
			// Output pagination
			if ( $echo ) {
				echo '<div class="wpex-pagination wpex-clr' . esc_attr( $align ) . '">' . paginate_links( $args ) . '</div>';
			} else {
				return '<div class="wpex-pagination wpex-clr' . esc_attr( $align ) . '">' . paginate_links( $args ) . '</div>';
			}
