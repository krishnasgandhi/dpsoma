...
function wpex_recommended_plugins() {
	return apply_filters( 'wpex_recommended_plugins', array(
		'total-theme-core'       => array(
			...
			'source'             => 'https://totalwptheme.s3.amazonaws.com/plugins/total-theme-core/version-1-0-3/total-theme-core.zip',
			'required'           => true,
		...
		'js_composer'          => array(
			...
			'source'           => 'https://totalwptheme.s3.amazonaws.com/plugins/wpbakery/version-6-0-3/js_composer.zip',
			'required'         => false,
			'force_activation' => false,
		),
		'templatera'           => array(
			'name'             => 'Templatera',
			'slug'             => 'templatera',
			'source'           => 'https://totalwptheme.s3.amazonaws.com/plugins/templatera.zip',
			'version'          => '2.0.1',
			'required'         => false,
			'force_activation' => false,
		),
...
function wpex_get_post_terms_list( $args = array() ) {
	...
	if ( $list_terms && is_array( $list_terms ) ) {
		$sep = $sep ? $sep : apply_filters( 'wpex_list_post_terms_sep', '<span class="wpex-sep">,</span> ', $instance );
...
if ( ! function_exists( 'wpex_get_star_rating' ) ) {
	function wpex_get_star_rating( $rating = '', $post_id = '', $before = '', $after = '' ) {
		...
		// Apply filters
		$output = apply_filters( 'wpex_get_star_rating', $output );
